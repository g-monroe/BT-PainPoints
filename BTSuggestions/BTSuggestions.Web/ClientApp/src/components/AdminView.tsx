                    ...
                    <table className="adminTable">
                        ...
                        <tbody>
                            {this.state.issues.map(i => (<tr key={i.painPointId}>
                                ...
                                <td className="megaColumn"><TextArea rows={2} key={"summary"} value={i.painPointSummary} onChange={(e: any) => this.handleInput(e, i.painPointId, "painPointSummary")}/></td>
                                <td className="megaColumn"><TextArea rows={2}  key={"annotation"} value={i.painPointAnnotation} onChange={(e: any) => this.handleInput(e, i.painPointId, "painPointAnnotation")}/></td>
                                <td className="thinColumn"><Select key={"type"} style={{ width: 100 }} dropdownMatchSelectWidth={false} defaultValue={i.painPointType}  onChange={(e: any) => this.handleSelect(e, i.painPointId, "painPointType", painPointList)}>{this.renderDropdowns(painPointList)}</Select></td>
                                ...
                                <td className="thinColumn"><Select key={"industry"} style={{ width: 150 }} dropdownMatchSelectWidth={false} defaultValue={i.industryType} onChange={(e: any) => this.handleSelect(e, i.painPointId, "industryType", industryList)}>{this.renderDropdowns(industryList)}</Select></td>
                                <td className="thinColumn"><Select key={"status"} style={{ width: 150 }} dropdownMatchSelectWidth={false} defaultValue={i.submissionStatus}  onChange={(e: any) => this.handleSelect(e, i.painPointId, "submissionStatus", statusList)}>{this.renderDropdowns(statusList)}</Select></td>
