...
import { Layout, Form as AntForm, Select, Input, Button, Slider, Icon, message, Card } from 'antd';
                            ...
                            <Form onSubmitCapture={handleSubmit} className="create-form">
                                <div className="required-components">
                                    ...
                                    <FormItem className="input" label="Issue Summary" required validateStatus={this.getValidationStatus(errors.summary)}>
                                        <TextArea id="summary" placeholder="Description of Problem" value={values.summary} onChange={handleChange} rows={1} />
                                    </FormItem>
                                    <FormItem className="input" label="Issue Annotation" validateStatus={this.getValidationStatus(errors.annotation)}>
                                        <TextArea id="annotation" placeholder="Personal Notes about Problem" value={values.annotation} onChange={handleChange} rows={1} />
                                    </FormItem>
                                    <FormItem className="input" label="Issue Type" required validateStatus={this.getValidationStatus(errors.types)}>
                                        <Select maxTagCount={3}  mode="multiple" defaultValue={['Bug']} className="select" id="types" onChange={(x: any) => setFieldValue("types", x)} value={values.types}>{this.renderDropdowns(painPointList)}</Select>
                                    </FormItem>
